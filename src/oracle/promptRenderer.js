...
function renderOracleMarkdown(brief) {
  ...
  return [
    ...
    '- Motion quality: purposeful motion only; avoid transition-all, ease-in UI timing, scale(0) entrances, layout-property animation, and motion without reduced-motion fallback.',
    '',
    '## Data and Backend',
...
function renderOffByOnePrompt(brief) {
  ...
  return [
    ...
    '- Interaction quality gate: motion must clarify hierarchy, reveal, state, or product flow; avoid transition-all, ease-in UI timing, scale(0) entrances, layout-property animation, center-origin anchored overlays, and missing reduced-motion handling.',
