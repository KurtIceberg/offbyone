...
v4.7.2 also carries a local **Motion Quality Gate** distilled from `emilkowalski/skills` into `professionalGuidance.motionQualityGate`. It gives OffByOne generation and supervisor passes concrete interaction-taste rules: purposeful motion only, strong ease-out/ease-in-out values, routine UI under 300ms, transform/opacity-first performance, trigger-aware overlay origins, `scale(0.95)+opacity` entrances instead of `scale(0)`, and reduced-motion fallbacks. The supervisor flags source-level motion red flags such as `transition-all`, `ease-in` UI timing, `scale(0)` entrances, layout-property animation, over-slow routine UI, center-origin anchored overlays, and motion without reduced-motion handling.
